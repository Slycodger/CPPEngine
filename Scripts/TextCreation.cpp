#define TextCreation_MAIN
#include "TextCreation.h"

using namespace TextCreation;
...
constexpr float Resolution = 2;


unsigned int BitmapTexture, FBO, Texture;
...
//Resizes the bitmap to fit just right in the screen
//Don't ask me how the math works, I forgot
void resizeBitmap(float Height, FT_Face& Face)
{
	float Layers = 0;
...
//Loads a character and converts values into texture coordinates
//Returns character height of tallest character
float loadAndReturnMaxHeight(FT_Face& Face)
{
	Height = 0;
	Under = 0;
...
//Loads a character set from ttf file, currently only arial is used
int loadCharacterSet(Shader &shader)
{
	...
	resizeBitmap(loadAndReturnMaxHeight(Face), Face);
	float Height = loadAndReturnMaxHeight(Face);
...
//Renders out whatever characters are within the buffer, 'Text'
...
//Creates the texture that will shows the text
